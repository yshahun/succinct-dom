...
/**
 * A succinct {@link Text} node. Internal {@code ordinalIndex} is treated as the index of the node
 * among all the text nodes.
 *
...
public class TextImpl extends AbstractCharacterData implements Text {
  ...
  /**
   * Constructs a succinct text node.
   *
   ...
   */
  public TextImpl(SuccinctDom dom, int index, int ordinalIndex) {
  ...
  public String getNodeValue() throws DOMException {
    return dom.getText(ordinalIndex);
  }
  ...
  public int getLength() {
    return dom.getTextLength(ordinalIndex);
  }

  @Override
  public String substringData(int offset, int count) throws DOMException {
    if (offset >= 0 && count >= 0) {
      String substr = dom.getTextSubstring(ordinalIndex, offset, count);
